...
class Agent:
    ...
    def __init__(self, env):
        ...
        self.beta = 0.01  # learning rate0.01
        self.gamma = 0.95  # reward discount factor0.95
    ...
    def get_action(self, env):
        ...
        if obs == 1 and ran==0:
            Q_s = self.Q[state[0], state[1], actions_allowed]
            actions_greedy = actions_allowed[np.flatnonzero(Q_s == np.max(Q_s))]
            actC = np.random.choice(actions_greedy)#defense
            if actC==act:
                indexAc=np.where(actions_allowed==actC)
                valueofactC=Q_s[indexAc[0]]
                DictAction={actionAl:value for actionAl in actions_allowed if actionAl!=actC
                      for value in Q_s if value==Q_s[np.where(actions_allowed==actionAl)[0]]}

                act=max(DictAction, key=DictAction.get)
            else:
                act=actC

            state_next = (state[0] + env.action_coords[act][0],
                          state[1] + env.action_coords[act][1])
            state_nextA = (stateA[0] + env.action_coords[actA][0],
                           stateA[1] + env.action_coords[actA][1])
            if state_nextA==state_next:
                print(state_next,state_nextA,Q_s,actions_allowed,actions_greedy,state,stateA,act,actA)
...
class Learn:
    def debut(self):
        ...
        for episode in range(N_episodes):
            ...
            Lst= [episode,reward_episode, int(j), reward_episodeA,ob]
